...
const TeamManagement: React.FC<TeamManagementProps> = ({ onMonitor }) => {
  ...
  // Helper to get role display name (keeping this for the card badge)
  ...
  const categoriesToShow = filter === 'Todos' ? visibleTabs.filter(t => t !== 'Todos') : [filter];
        ...
        {loading ? (
          ...
          <>
            {categoriesToShow.map((category) => {
              // Find sector and members
              const sector = sectors.find(s => s.name === category);
              const isSemSetor = category === 'Sem Setor';

              const categoryMembers = members
                .filter(member => {
                  const isActiveMatch = member.isActive !== false;
                  const sectorMatch = isSemSetor
                    ? !member.sectorId
                    : (member.sectorId === sector?.id || member.sector?.name === category);
                  return isActiveMatch && sectorMatch;
                })
                .filter(member =>
                ...
                <div key={category} className="animate-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    <div
                      ...
                      style={{ backgroundColor: sector?.color || '#94a3b8' }}
                    ></div>
                    ...
                    {categoryMembers.map((member) => (
                      ...
                      >
                        ...
                        <div className="flex flex-col items-center mb-6">
                          <div className="w-24 h-24 rounded-[2.5rem] bg-slate-50 border border-slate-100 flex items-center justify-center font-black text-rose-600 text-3xl group-hover:scale-105 transition-transform duration-500 mb-4">
                          ...
                          {/* Role Badge */}
                          <div className={`mt-2 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${getRoleStyle(member.role)}`}>
